...
import { Box, Typography, Button, IconButton, Divider, Table, TableBody, TableCell, TableHead, TableRow, Select, MenuItem, FormControl, InputLabel, TextField, Stack, Tooltip, Chip } from "@mui/material";
...
function DefectDetectionHitlContent({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    backendApi.getInternalReviewDetail(sessionId!)
      .then(json => {
        ...
        const existingHuman = pipelineData["defect_detection_human_json"];
        const aiOutput = pipelineData["defect_detection_ai_json"];
        ...
        } else if (rawOutput && rawOutput.defect_detections) {
          outputArray = [rawOutput];
        } else {
        ...
        outputArray = outputArray.map(frame => ({
            ...frame,
            defect_detections: (frame.defect_detections || []).map((d: any, i: number) => ({
                ...d,
  ...
  const loadFrameData = (frames: any[], index: number) => {
    ...
    // Setup Detections
    const initialDetections = frame.defect_detections || [];
  ...
  const pushToHistory = (newDetections: any[]) => {
    ...
    setFullOutput(prev => {
        const arr = [...prev];
        arr[currentFrameIndex] = { ...arr[currentFrameIndex], defect_detections: newDetections };
        return arr;
    });
  };
  ...
  const undo = () => {
    if (historyStep > 0) {
      ...
      setFullOutput(prev => {
          const arr = [...prev];
          arr[currentFrameIndex] = { ...arr[currentFrameIndex], defect_detections: oldDets };
          return arr;
      });
    }
  };
  ...
  const redo = () => {
    if (historyStep < history.length - 1) {
      ...
      setFullOutput(prev => {
          const arr = [...prev];
          arr[currentFrameIndex] = { ...arr[currentFrameIndex], defect_detections: newDets };
          return arr;
      });
    }
  };
  ...
  const handleSave = async (dataToSave: any) => {
    try {
      await backendApi.saveDefectDetectionReview(sessionId!, {
        decision: "save_assessment",
        corrections: dataToSave,
        reviewer: "System Admin"
      });
      console.log("Saved defect detection:", dataToSave);
    } catch (err) {}
  };
        ...
        <HitlWorkspace>
          ...
          <HitlFilmstrip>
            {fullOutput.map((frame, i) => {
              ...
              const detsCount = frame.defect_detections?.length || 0;
            ...
            {selectedDet ? (
              <Stack spacing={2.5}>
                <FormControl fullWidth size="small">
                  <InputLabel sx={{color:'#64748b'}}>Defect Type</InputLabel>
                  <Select
                    value={selectedDet.class_name || ""}
                    label="Defect Type"
                    sx={{ color: '#0f172a', '.MuiOutlinedInput-notchedOutline': { borderColor: '#64748b' } }}
                    onChange={(e) => updateDetection(selectedDet.id, { class_name: e.target.value })}
                  >
                    <MenuItem value="DEFORMATION">Deformation</MenuItem>
...
export function DefectDetectionHitlPage() {
  const { sessionId } = useParams();
  if (!sessionId) return null;
  return (
    <HitlLayout>
      <DefectDetectionHitlContent sessionId={sessionId} />
    </HitlLayout>
  );
}
